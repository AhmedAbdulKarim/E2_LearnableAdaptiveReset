...
class AdaptiveMultiplicativeResetLIF(nn.Module):
    def __init__(self, tau=2.0, v_threshold=1.0, init_w_res=0.0):
        super().__init__()
        self.tau = tau
        self.v_threshold = v_threshold
        self.v = 0.0
        
        # 1. THE NOVELTY: A continuous learnable weight for the reset
    ...
    def forward(self, x):
        ...
        # 2. THE HARDWARE SAFETY CLAMP (Crucial)
        # We pass w_res through a sigmoid to guarantee the multiplier stays between 0.0 and 1.0.
        # If the multiplier exceeds 1.0, the neuron will amplify its voltage every time it spikes,
        # creating a deadly positive feedback loop that results in NaNs.
        safe_reset_weight = torch.sigmoid(self.w_res)

        for t in range(T):
            # A. Charge (Standard LIF integration)
            self.v = self.v + (1.0 / self.tau) * (x[t] - self.v)

            # B. Fire (Generate Binary Spike)
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        self.mlp2_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
...
class Token_QK_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.q_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
        
        self.k_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        #self.k_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        self.k_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)

        #self.attn_lif = MultiStepLIFNode(tau=2.0, v_threshold=0.5, detach_reset=True, backend='torch')
        self.attn_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=0.5)

        self.proj_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1)
        self.proj_bn = nn.BatchNorm1d(dim)
        #self.proj_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        self.proj_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
...
class Spiking_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.v_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
        self.attn_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=0.5)
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj1_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
        ...
        self.proj2_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
        ...
        self.proj3_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
        ...
        self.proj_res_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
...
class PatchEmbeddingStage(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj4_lif = AdaptiveMultiplicativeResetLIF(tau=2.0, v_threshold=1.0)
